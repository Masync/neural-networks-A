...
function main() {
  ...
  **/
  ...
  function validation(data) {
    ...
    // * @param {bool,number} y return resultado de la compuerta NAND
    function nand(comp1,_w1,comp2,_w2,comp3,_umbral) {
      let data = (((comp1 * (-_w1)) + (comp2 * (-_w2)) + (comp3 * _umbral)));
      return resul = validation(data);
  
    };
    // * @param {bool,number} y return resultado de la compuerta  AND y NAND
    function Or(comp1,resulAnd1,comp2,resulNand1,comp3,umbral) {
      let data = ((comp1 + resulAnd1) + (comp2 + resulNand1) + (comp3 + umbral));
      return resul = validation(data);
    };
     // * @param {string,number} y return el resultado de las compuerta XOR en HTML
  ...
  //* Cargando valores en la funcion nand()
  let nand1 = nand(0,_w1,0,_w2,1,_umbral);
  let nand2 = nand(0,_w1,1,_w2,1,_umbral);
  let nand3 = nand(1,_w1,0,_w2,1,_umbral);
  let nand4 = nand(1,_w1,1,_w2,1,_umbral);

  //* Cargando valores en la funcion Or() con los resulados de and() y nand()
  let _or1 = Or(0,and1,0,nand1,1,umbral);
  let _or2 = Or(0,and2,1,nand2,1,umbral);
  let _or3 = Or(1,and3,0,nand3,1,umbral);
  let _or4 = Or(1,and4,1,nand4,1,umbral);
